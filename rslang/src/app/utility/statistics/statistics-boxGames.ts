...
export default class StatisticsBoxGames {
  ...
  render(): HTMLElement {
    ...
    const allWordsSprint = this.storage.optional.SprintCountAnswerWrong
    + this.storage.optional.SprintCountAnswerСorrect;

    const allWordsAudio = this.storage.optional.AudioCountAnswerWrong
    ...
    if (this.text === 'Sprint') {
      ...
      new BaseComponent(this.statisticsBoxGames, 'div', ['main__statistics__boxGames-row3'], `<span class="game-inRow">${this.storage.optional.SprintInRow}</span><span>  in a row</span>`).render();
    }
    ...
    if (this.text === 'Audio Challenge') {
      ...
      new BaseComponent(this.statisticsBoxGames, 'div', ['main__statistics__boxGames-row3'], `<span class="game-inRow">${this.storage.optional.AudioInRow}</span><span>  in a row</span>`).render();
    }
