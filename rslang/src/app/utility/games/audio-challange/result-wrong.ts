...
export default class StatWordsWrong {
  ...
  render(): HTMLElement {
    ...
    for (let i = 0; i < this.storage.namesAnswerWrong.length; i += 1) {
      new BaseComponent(this.statWordsWrong, 'section', ['result-wrong__container'], `<div class = "result__sound-wrong"  data-ind = "${i}"></div><span class = "result__word">${this.storage.namesAnswerWrong[i]}</span><span class = "result__trans"> - ${this.storage.namesAnswerWrongTranslate[i]}</span>`)
        .render();
    }

    const btnSound: NodeListOf<Element> = document.querySelectorAll('.result__sound-wrong');
    if (btnSound) {
      btnSound.forEach((el) => el.addEventListener('click', ({ target }) => this.playSoundWordWrong(target as HTMLElement)));
    }
