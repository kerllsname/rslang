...
export default class MainGames {
  ...
  render(): void {
    ...
    new BaseComponent(this.audioGame, 'div', ['game-audio__description'], 'A word is voiced to you, and you choose the correct translation').render();
    new BaseComponent(this.audioGame, 'button', ['game-audio__play'], 'play').render().addEventListener('click', ({ target }) => this.pushBtnPlayAudio(target as HTMLElement));
    ...
    new BaseComponent(this.sprintGame, 'div', ['game-sprint__description'], 'In 10 seconds you have to answer the maximum number of correctly translated words').render();
    new BaseComponent(this.sprintGame, 'button', ['game-sprint__play'], 'play').render().addEventListener('click', ({ target }) => this.pushBtnPlaySprint(target as HTMLElement));
  }
}
